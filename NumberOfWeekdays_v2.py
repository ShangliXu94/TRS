...
class CountWeekdays:
    ...
    @staticmethod
    def calculate_days_in_between(start_year, start_month, start_day, end_year, end_month, end_day):
        '''
        benchmark_date = '1900-01-01' # It's a Monday
        '''
        ...
        days_difference = 0
        # calcualte date difference from start_year-01-01
        ...
        for m in range(1, end_month):
            days_difference += CountWeekdays.get_days_from_month(end_year, m)

        days_difference += (end_day - 1)
        
        # calcualte date difference from start_year-start_month-start_day
        for m in range(1, start_month):
            days_difference -= CountWeekdays.get_days_from_month(start_year, m)

        days_difference -= (start_day - 1)
    ...
    def count_weekdays(self):
        if CountWeekdays.check_date_format(self.start_date) and CountWeekdays.check_date_format(self.end_date):
            ...
            days_difference = CountWeekdays.calculate_days_in_between(start_year, start_month, start_day, end_year, end_month, end_day)
